...
class NewPvEWorldWindowControl(DirectControls):
    
    def __init__(self, parent):

        DirectControls.__init__(self, parent)
#        self.accept('enter', parent.submit, [])
...
class NewPvEWorldWindow:
    ...
    def __init__(self, parent):
        ...
        self.control = NewWorldWindowListener(self)
        self.createOptionsMenus()
        self.control=NewPvEWorldWindowControl(self)
...
#        for i in range(2):
    ...
    def receiveMessageFromServer(self, success, name = ''):
        print 'pve receive message: '+str(success)
        if success == 0:
            self.unload()
            obj = {'worldName': name, 'password': ''}
    ...
    def createOptionsMenus(self):
        ...
        self.optionMenus[0]['items'] = ['Savanna']
    ...
    def submit(self, param = None):
        ...
        rContents = {'worldType' : 1, 'worldName': worldName, 'ecosystem' : self.ecosystem, 
                   'maxPlayerNumber': 100, 'privacyType' : self.privateSet,
                   'password' :  password}
        main.cManager.sendRequest(Constants.CMSG_CREATE_NEW_WORLD, rContents)

        main.createMessageBox(2, 'Creating World...')
