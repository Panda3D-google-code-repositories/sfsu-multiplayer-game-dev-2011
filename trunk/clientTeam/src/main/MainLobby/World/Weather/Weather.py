...
class WeatherControl(DirectControls):
    def __init__(self, parent):

        DirectControls.__init__(self, parent)
        self.accept('control-w', parent.toggleVisibility) 
...
#        self.control = WeatherControl(self)
...
#                                        text_font = Constants.FONT_TYPE_01,
    ...
    def createWeatherPrediction(self):
        ...
        if self.CURRENT_PREDICTION == 0:
            ...
            self.weatherFrame = self.threeDaysFrame
        elif self.CURRENT_PREDICTION == 1:
            self.createSevenDaysPrediction()
            self.weatherFrame = self.sevenDaysFrame
        elif self.CURRENT_PREDICTION == 2:
            self.create14DaysPrediction()
            self.weatherFrame = self.daysFrame14
        elif self.CURRENT_PREDICTION == 3:
            self.create21DaysPrediction()
            self.weatherFrame = self.daysFrame21
        elif self.CURRENT_PREDICTION == 4:
            self.create28DaysPrediction()
            self.weatherFrame = self.daysFrame28
        
    def toggleVisibility(self):
        
        if self.weatherFrame.isHidden():
            self.weatherFrame.show()
        else:
            self.weatherFrame.hide()
        
    def hideWeatherFrame(self):
        self.weatherFrame.hide()
                 
    def showWeatherFrame(self):
        if not (self.weatherFrame is None):
            self.weatherFrame.show()
