...
class LobbyHeaderListener(DirectObject):
    
    def __init__(self, parent):
        self.accept(Constants.LISTENER_GAME_LOBBY_RESPONSE, parent.responseFromServer, [])
...
#        thread1 = LoadGameLobby(self.mainFrame)
#        thread1 = LoadGameLobby(self.mainFrame)
#        self.pvpGameLobby= thread1.loadPvPGameLobby()
#        self.pveGameLobby =thread1.loadPvEGameLobby()
        ...
        self.listener = LobbyHeaderListener(self)
        self.lobby.show()
        
    def printMessageFromMsgQ(self, msg):
        print msg;
    ...
    def createSearchEntry(self):
        ...
        self.searchEntry = DirectTextField( self.mainFrame,
                                          ...
                                          width = 90,
                                          command = self.submitSearchResult,
    ...
    def submitSearchResult(self, dummy=None):
        
        query = self.searchEntry.get().strip()
        print 'serach for :' + query
        if query != '':
           
            MessageBoxWithEntry(query)
    
    def responseFromServer(self, success, status):
...
#        messenger.send(Constants.UPDATE_PVE_ONLINE_PLAYERS, [_list])
    ...
    def unload(self):
        self.bg.destroy()
        self.pvpGameLobby.unload()
        self.pveGameLobby.unload()
        self.mainFrame.destroy()
        self.listener.removeAllTasks()
        
class LoadGameLobby(Thread):
    ...
    def loadPvPGameLobby(self):
        
        return PvPGameLobby(self.mainFrame)
    
    def loadPvEGameLobby(self):
        
        return PvEGameLobby(self.mainFrame)
