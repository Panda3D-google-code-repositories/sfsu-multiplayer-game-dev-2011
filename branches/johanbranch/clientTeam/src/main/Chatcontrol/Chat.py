...
#            self.chatBubble.clear()
    ...
    def onChatFocus(self):

        self.chatEntry['frameColor'] = (0.8, 0.8, 0.8, 0.9)
        self.chatMainFrame.getControls().enable()

    def onChatFocusOut(self):

        self.chatEntry['frameColor'] = (0.8, 0.8, 0.8, 0.4)
        self.chatMainFrame.getControls().disable()

        if self.lastType < 0:
            #self.world.charHero.chatBubble.clear()
            self.chatBubble.clear()
            
#    def setButtonFocus(self, index):
#        print 'setButtonFocus, button len '+str(len(self.chatButtons))
#        for i in range (len(self.chatButtons)):
#            if i == index:
#                print 'set button focus'
#                self.chatButtons[i]['frameColor']=(1, 1, 1, 1)
#            else:
#                self.chatButtons[i]['frameColor']=Constants.CHAT_BUTTON_COLOR
        
        
    def handleHideButton(self):

        if self.hideButton['text'] == '-':
            self.hideButton['text'] = '+'

            self.hideContents()
        elif self.hideButton['text'] == '+':
            self.hideButton['text'] = '-'

            self.showContents()

    def handleHoverEnter(self):

        if self.hideButton['text'] == '-':
            self.hideButton.setMouseText('Hide')
        elif self.hideButton['text'] == '+':
            self.hideButton.setMouseText('Show')

    def showContents(self):

        self.titleBar['frameSize'] = (-0.025, 0.025, -0.24, 0.24)

        self.chatMainFrame.reparentTo(aspect2d)
...
#                    _type = Constants.CMSG_PVPWORLD_CHAT
                    ...
                    msg = msg[1: len(msg)].strip()
#                elif msg.startswith(self.pvpOp):
                elif _type == Constants.CMSG_PVPGAME_CHAT and len(msg)>1:
                    print 'parseMsg: pvp game chat'
#                    _type = Constants.CMSG_PVPGAME_CHAT
                    msg = msg[1 : len(msg)].strip()

#                    hasText = True
#                    if msg.find(' ') != -1:
#                        targetUser = msg[0 : msg.find(' ')]
#                    else:
#                        targetUser = msg
#                        hasText = False
...
#                    if send:
#                        if len(targetUser) > 0:
#                            self.newpvpOp = self.pvpOp + targetUser + ' '
#                        else:
#                            self.newpvpOp = self.pvpOp

#                    if hasText:
#                        if send and targetUser.lower() == self.world.charHero.getName().lower():
#                            _type = -1
#                            result = DatabaseHelper.dbSelectRowByID('msg', 'msg_id', 5)
#                            msg = result['msg_text']
#                        else:
...
#            else:
    ...
    def addMessage(self, name, _type, msg):
        ...
        for mObject in pMessage:
            ...
            self.chatLog.append(ChatObject(name, _type, mObject))
